...
namespace Hive.Application.Services.Meta.CampaingMapper
{
    public class MetaCampaignStrategyMapper : IMetaCampaignStrategyMapper
    {
        ...
        public async Task<string> CreateMetaCampaign(CampaignStrategy campaingStrategy, string accessToken)
        {
            var campaignMeta = MapCampaignMeta(campaingStrategy.Campaign!);
;
            var campaignId = await _metaApiService.CreateCampaign(campaignMeta, accessToken);
        ...
        public async Task<MetaAdSetConfig> MapToAdset(CampaignStrategy campaingStrategy, string campaignId, string accessToken)
        {
            ...
            {
                var interests = await _metaApiService.SearchInterestsBatch(campaingStrategy.TargetingStrategy.Audience.Interests, accessToken);

                var locations = await MapLocationsMeta(campaingStrategy.TargetingStrategy.Audience.Locations, accessToken);

                var adSetMeta = strategyImpl.BuildAdSetAsync(campaingStrategy, campaignId, locations, interests.Value!);
        ...
        public async Task<MetaGeoLocationsConfig> MapLocationsMeta(List<Location> locations, string accessToken)
        {
            ...
            // Você pode querer lidar com os erros dos resultados que falharam aqui.
            // var failedRegions = regionResults.Where(result => result.IsFailure);
            // ... logar ou retornar os erros ...
